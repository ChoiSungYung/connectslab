...
def send_s3_file_to_kafka(bucket_name, object_key):
    try:
        ...
        # 파일명에서 확장자를 제거한 고유 ID로 사용
        file_id = os.path.splitext(os.path.basename(object_key))[0]  # 확장자 제거한 파일명만 추출
        ...
        while True:
            ...
            headers = [
                ...
                ('file_id', file_id.encode('utf-8'))  # 확장자 없는 파일명
            ]

            # 각 청크를 Kafka로 전송 (헤더 포함)
            future = producer.send('test6', key=object_key.encode('utf-8'), value=chunk, headers=headers)
